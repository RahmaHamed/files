...
void *searchServerThread(void *sst) {
    ...
    while ((bytes = read(sock, buffer + total, buffer_size - total)) > 0) {
        total += bytes;
...
void handle_client(int cl_connect, struct sockaddr_in client_addr,char *ip1,int port1,char *ip2,int port2,char *ip3,int port3) {
    ...
    int valread = read(cl_connect, buffer, MAX_SIZE - 1);
        if (valread <= 0) {
        close(cl_connect);
        return;
    }
    ...
    pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
        SearchParams params1 = { buffer, &search1, ip1, port1, &mutex };
    SearchParams params2 = { buffer, &search2, ip2, port2, &mutex };
    SearchParams params3 = { buffer, &search3, ip3, port3, &mutex };
        pthread_create(&t1, NULL, searchServerThread, &params1);
    pthread_create(&t2, NULL, searchServerThread, &params2);
    pthread_create(&t3, NULL, searchServerThread, &params3);
